...
,"--leader-elect=true","--kubeconfig=/var/lib/kubernetes/controller_kubeconfig","--root-ca-file=/var/lib/kubernetes/ca.pem","--service-account-private-key-file=/var/lib/kubernetes/kubernetes-key.pem"
,"--service-cluster-ip-range=${svc_cluster_ip_range}","--v=2"]
  - name: kube-scheduler
    ...
    command: ["kube-scheduler","--leader-elect=true","--kubeconfig=/var/lib/kubernetes/scheduler_kubeconfig","--v=2"]
  - name: flannel
    binds:
     - /var:/var:rshared,rbind
     - /etc/resolv.conf:/etc/resolv.conf
...
,"--container-runtime=docker","--cluster-domain=${cluster_domain}","--cluster-dns=${cluster_dns}","--docker=unix:///var/run/docker.sock","--kubeconfig=/var/lib/kubernetes/kubelet_kubeconfig"
,"--serialize-image-pulls=false","--v=2"]
  - name: kube-proxy
    ...
    capabilities:
     - all
    command: ["kube-proxy","--cluster-cidr=${cluster_cidr}","--kubeconfig=/var/lib/kubernetes/kubelet_kubeconfig","--proxy-mode=iptables","--v=2"]
files:
  - path: etc/kubernetes
    directory: true
    mode: "0777"
  ...
  - path: etc/kubernetes/authorization-policy.jsonl
    source: ~/authorization-policy.jsonl
